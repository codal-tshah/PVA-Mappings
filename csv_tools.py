...
def normalize_field_name(name, return_parts=False):
    ...
    1. Lowercase and split camelCase.
    2. Removes common prefixes and connectors.
    3. Handles real estate abbreviations consistently.
    4. Singularizes terms and removes stop words to handle natural language variations.
    """
    if not name:
    ...
    spaced_name = re.sub(r'([a-z])([A-Z])', r'\1 \2', orig_name)
            ...
            for cat, keywords in categories.items():
                ...
                for kw in keywords:
                    ...
                    if kw in combined_text:
                        score += 1
                        # Boost score if the keyword appears in specific columns
                        for col in ['Merged ERD Fields', 'Field CoStar Data', 'Field LightBox Data']:
                            if col in row and kw in str(row[col]).lower():
                                score += 2
